...
public class OBJLoader {
    ...
    private static final String MODEL_PATH = GlobalConstants.getResourcePath() + "/model/";
    ...
    private static float convertDataToArrays(List<Vertex> vertices, List<Vector2f> textures,
        ...
        for (int i = 0; i < vertices.size(); i++) {
            ...
            Vector3f position = currentVertex.getPos();
            Vector2f textureCoord = textures.get(currentVertex.getTexIndex());
    ...
    private static void dealWithAlreadyProcessedVertex(Vertex previousVertex, int newTextureIndex,
        ...
        if (previousVertex.hasSameTexAndNormal(newTextureIndex, newNormalIndex)) {
            indices.add(previousVertex.getIndex());
        } else {
            Vertex anotherVertex = previousVertex.getDuplicatedVertex();
            if (anotherVertex != null) {
                dealWithAlreadyProcessedVertex(anotherVertex, newTextureIndex, newNormalIndex,
                        indices, vertices);
            } else {
                Vertex duplicateVertex = new Vertex(vertices.size(), previousVertex.getPos());
                duplicateVertex.setTexIndex(newTextureIndex);
                duplicateVertex.setNormalIndex(newNormalIndex);
                previousVertex.setDuplicatedVertex(duplicateVertex);
